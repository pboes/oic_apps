...
export class OICFramework {
  ...
  async generateQR(amount, appId, customData = "") {
    try {
...
export function useOICFramework() {
  ...
  return {
    ...
    isConnected,
    events,
    generateQR: framework.generateQR.bind(framework),
...
export const OICCriteria = {
  ...
  userPayment: (sender, amount, appId) => ({
    recipient: "0xf48554937f18885c7f15c432c596b5843648231d",
    sender: sender,
...
// Helper function to create an OIC app component
export function createOICApp(config) {
  return function OICApp() {
    const { framework, isConnected, events, generateQR, registerEventHandler } =
      useOICFramework();
    const [appState, setAppState] = useState(config.initialState || {});

    // Register event handlers on mount
    useEffect(() => {
      if (config.eventHandlers) {
        config.eventHandlers.forEach((handler, name) => {
          registerEventHandler(name, handler.criteria, (eventData) => {
            handler.callback(eventData, appState, setAppState);
          });
        });
      }
      ...
      return () => {
        if (config.eventHandlers) {
          config.eventHandlers.forEach((_, name) => {
            framework.unregisterEventHandler(name);
          });
        }
      };
    }, [framework, registerEventHandler]);
