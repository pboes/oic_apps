...
import { useOICFramework, OICCriteria, OICStyles } from '../../lib/oic-framework';
import { useState, useEffect } from 'react';

const APP_ID = 'random-number';

export default function RandomNumberApp() {
  const { isConnected, generateQR, registerEventHandler, unregisterEventHandler } = useOICFramework();
      ...
      <div style={OICStyles.content}>
          ...
          <div style={{
            ...
            {lastTransaction && (
              <small style={{color: '#666'}}>
                Generated from transaction: {lastTransaction.transactionHash?.substring(0, 10)}...
              </small>
            )}
          </div>
        )}
        ...
        {qrCode && (
          <div style={OICStyles.qrContainer}>
            <h3 style={{...OICStyles.h3, marginTop: 0}}>
              {isWaiting ? 'Scan to Generate Random Number' : 'QR Code Ready'}
            </h3>
            <img
              src={qrCode}
        ...
        <div style={OICStyles.debug}>
          ...
          <ul style={{margin: '10px 0', paddingLeft: '20px'}}>
            <li>Scan the QR code with a compatible wallet</li>
            <li>Send exactly 1 CRC to the contract</li>
            <li>Include "{APP_ID}" in the data field</li>
            <li>A random number will be generated when payment is received</li>
          </ul>

          <strong>Technical Details:</strong>
          <ul style={{margin: '10px 0', paddingLeft: '20px'}}>
            <li>Recipient: 0xf48554937f18885c7f15c432c596b5843648231d</li>
            <li>Amount: 1000000000000000000 (1 CRC in 18 decimals)</li>
            <li>Data: {APP_ID}</li>
          </ul>
        </div>
      </div>
